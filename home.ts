const form = document.querySelector('#recados') as HTMLFormElement;
const divErro = document.querySelector("#msg-erro") as HTMLDivElement;
const tabela = document.querySelector('#tbody') as HTMLBodyElement;
let idx = form.idx.value as HTMLInputElement;
...
const atualizarLocalStorage = (recados: Recado[]) => {
...
const removerRecado = (id) => {
    const recados = recuperarLocalStorage();
    const indexRecado = recados.findIndex((recado : Recado) => recado.id === id);
    if(indexRecado < 0)
        return;     
